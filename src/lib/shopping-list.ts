...
export function getShoppingList(mealDays: MealDate[], recipes: Database<Recipe>, ingredients: Database<Ingredient>): ShoppingList {
    ...
    for (const recipeName of recipeNames) {
        ...
        for (const ingredient of recipe.ingredients) {
            ...
            if (ingredientData.fresh) {
                const daysWithIngredient = mealDays.filter(meal => meal.meal === recipeName).map(meal => meal.date);
