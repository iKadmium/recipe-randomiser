...
import { getRandom, getRandomWithPenalty, toUTCIsoString } from "./util";
...
export function getMealForDate(date: Date, easyMealDays: Date[], takeoutDays: Date[], recipes: Database<Recipe>, existingMeals: MealDate[]): string | undefined {
    const dayOfWeek = date.getDay();
    const isWeekend = dayOfWeek === 0 || dayOfWeek === 6; // Sunday or Saturday

    const dateString = toUTCIsoString(date);
    const isEasyMealDay = easyMealDays.some(easyMealDate => toUTCIsoString(easyMealDate) === dateString);
    const isTakeoutDay = takeoutDays.some(takeoutDate => toUTCIsoString(takeoutDate) === dateString);
    ...
    filteredRecipes = filteredRecipes.filter(recipe => {
        const usedCount = existingMeals.filter(meal =>
