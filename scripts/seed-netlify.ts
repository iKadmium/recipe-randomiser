 ...
 */
async function uploadToNetlify<T extends DataType>(key: string, data: Database<T>): Promise<void> {
	...
	if (
		...
		process.env.NETLIFY_BLOBS_STORE_TOKEN
	) {
...
function validateEnvironment(): void {
	...
	console.log(
		`  NETLIFY_BLOBS_STORE_TOKEN: ${process.env.NETLIFY_BLOBS_STORE_TOKEN ? '[SET]' : '[NOT SET]'}`
	);
	console.log('');
	...
	if (isLocalDev) {
		...
		console.log(
			'💡 No Netlify Blobs seeding needed - your app will use the file system data source'
		);
...
export async function seedNetlifyBlobs(): Promise<MigrationSummary> {
	...
	const isNetlifyLocalDev =
		process.env.RUNTIME_ENVIRONMENT === 'netlify' && process.env.CONTEXT === 'dev';
